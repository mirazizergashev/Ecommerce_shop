...
module.exports = CreateTransaction =async(data,javob)=>
{
   if(!(data.params.account.user))  {
    ...
    pool.promise().query("SELECT  * FROM transactions WHERE order_id=? limit 1; SELECT  * FROM orders WHERE id=? limit 1",
    [order,order])
    .then(async(rest)=>
    {
       if(rest[0].length>0)
       {
          if(rest[0][0].state !== 1 ) return javob.json({error:BilingErrors.UnexpectedTransactionState()});                
          if(rest[0][0].transaction_id != data.params.id ) return javob.json({error:BilingErrors.OrderNotFound()});   
          if(data.params.amount)
          {  console.log("bu",rest[1][0].amount , data.params.amount)
              if(!(parseInt(rest[1][0].amount)*100== data.params.amount)) return javob.json({error:BilingErrors.IncorrectAmount()});       
          }
       ...
       else
       {
        ...
        if(data.params.amount)
            { if(!(parseInt(rest[1][0].amount)*100===data.params.amount))
